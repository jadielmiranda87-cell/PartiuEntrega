...
import { getCashbackTransactions, CashbackTransaction } from '@/services/cashbackService';

type Tab = 'profile' | 'cashback';

export default function MotoboyProfileScreen() {
  const { motoboyProfile, profile, signOut } = useAppAuth();
  ...
  const handleShare = async () => {
    if (!motoboyProfile?.referral_code) return;
    await Share.share({
      message: `Use meu código de indicação no MotoLink e me ajude a ganhar cashback: ${motoboyProfile.referral_code}`,
    });
  };
        ...
        >
          ...
          <View style={styles.referralCard}>
            ...
            <View style={styles.referralCodeRow}>
              <Text style={styles.referralCode}>{motoboyProfile.referral_code ?? '--------'}</Text>
        ...
        >
          ...
          <View style={styles.howItWorksCard}>
            ...
            <TouchableOpacity style={styles.shareBtn2} onPress={handleShare} activeOpacity={0.8}>
              ...
              <Text style={styles.shareBtnText}>Compartilhar código: {motoboyProfile.referral_code}</Text>
            </TouchableOpacity>
          </View>
