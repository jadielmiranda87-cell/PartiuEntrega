...
// ── CNH Física: apenas câmera (sem galeria) ───────────────────────────────────
function CnhPhysicalCameraBtn({
  label, sublabel, onCamera, preview,
}: {
  label: string;
  sublabel?: string;
  onCamera: () => void;
    ...
    <View style={docStyles.wrapper}>
      ...
      {preview ? (
        ...
        <View style={docStyles.placeholder}>
          ...
          <Text style={docStyles.placeholderText}>Nenhuma foto tirada</Text>
          <Text style={docStyles.placeholderSub}>Somente câmera — importar da galeria não permitido</Text>
        </View>
      )}
      <TouchableOpacity style={[docStyles.btn, { flex: 1 }]} onPress={onCamera} activeOpacity={0.8}>
        <MaterialIcons name="camera-alt" size={18} color={Colors.primary} />
        <Text style={docStyles.btnText}>Tirar foto com a câmera</Text>
      </TouchableOpacity>
    </View>
  );
}

// ── Selfie Picker Button (somente câmera frontal) ─────────────────────────────
function SelfiePickerBtn({
  label, sublabel, onCamera, preview,
}: {
  label: string;
  sublabel?: string;
  ...
  return (
    <View style={docStyles.wrapper}>
      ...
      {preview ? (
        ...
        <View style={docStyles.placeholder}>
          <MaterialIcons name="face" size={36} color={Colors.textMuted} />
          <Text style={docStyles.placeholderText}>Nenhuma selfie tirada</Text>
          <Text style={docStyles.placeholderSub}>Use a câmera frontal</Text>
        </View>
      )}
      <TouchableOpacity style={[docStyles.btn, { flex: 0, paddingHorizontal: Spacing.xl }]} onPress={onCamera} activeOpacity={0.8}>
        <MaterialIcons name="camera-alt" size={18} color={Colors.primary} />
        <Text style={docStyles.btnText}>Tirar selfie com a câmera</Text>
      </TouchableOpacity>
    </View>
  );
}

// ── PDF Picker Button (CNH Digital) — Drive ou importar, somente PDF ───────────
function PdfPickerBtn({
  label, sublabel, onGoogleDrive, onGallery, preview,
}: {
  label: string;
  sublabel?: string;
  onGoogleDrive: () => void;
  ...
  return (
    <View style={docStyles.wrapper}>
      <Text style={styles.label}>{label}</Text>
      {sublabel ? <Text style={docStyles.sublabel}>{sublabel}</Text> : null}
      <Text style={docStyles.sublabel}>Somente arquivo PDF oficial — fotos e prints não são aceitos</Text>
      {preview ? (
      ...
      <TouchableOpacity style={docStyles.pdfBtn} onPress={onGoogleDrive} activeOpacity={0.8}>
        <MaterialIcons name="cloud" size={18} color="#4285F4" />
        <View style={{ flex: 1 }}>
          <Text style={[docStyles.pdfBtnTitle, { color: '#4285F4' }]}>Abrir no Google Drive</Text>
          <Text style={docStyles.pdfBtnSub}>Selecionar PDF salvo no Drive</Text>
        </View>
        <MaterialIcons name="chevron-right" size={18} color={Colors.textMuted} />
      </TouchableOpacity>

      <TouchableOpacity style={docStyles.pdfBtn} onPress={onGallery} activeOpacity={0.8}>
        <MaterialIcons name="photo-library" size={18} color={Colors.primary} />
        <View style={{ flex: 1 }}>
          <Text style={docStyles.pdfBtnTitle}>Importar da galeria</Text>
          <Text style={docStyles.pdfBtnSub}>Selecionar PDF do dispositivo — somente PDF</Text>
        </View>
        <MaterialIcons name="chevron-right" size={18} color={Colors.textMuted} />
      </TouchableOpacity>
    </View>
...
export default function RegisterMotoboyScreen() {
  ...
  const handleRegister = async () => {
    ...
    if (cnhType === 'physical') {
      if (!cnhFront) { showAlert('CNH frente obrigatória', 'Tire a foto da frente da CNH.'); return; }
      if (!cnhBack) { showAlert('CNH verso obrigatório', 'Tire a foto do verso da CNH.'); return; }
    } else {
      if (!cnhPdf) { showAlert('CNH digital obrigatória', 'Envie o arquivo PDF da CNH digital.'); return; }
    }
    if (!selfie) { showAlert('Selfie obrigatória', 'Tire uma selfie para verificação de identidade.'); return; }
    if (!lgpdAccepted || !privacyAccepted) {
      showAlert('Consentimento obrigatório', 'Aceite os termos LGPD e a Política de Privacidade para continuar.');
      return;
    }
    setLoading(true);
  ...
  const handleVerifyOTP = async () => {
    ...
    // Upload documents (já coletados no form) e ir para pagamento
    await performDocumentUpload(userId, mbData.id);
  };

  const performDocumentUpload = async (uid: string, motoboyId: string) => {
    setUploading(true);
    let cnhFrontUrl = '';
    let cnhBackUrl = '';
    let cnhPdfUrl = '';
  ...
  const handlePickCnhFrontCamera = async () => {
    ...
    else showAlert('Permissão necessária', 'Permita o acesso à câmera para tirar a foto.');
  };
  ...
  const handlePickCnhPdf = async () => {
    ...
    if (!result) {
      showAlert('Formato inválido', 'Somente arquivos PDF são aceitos para CNH digital. Fotos e prints não são permitidos.');
      return;
    }
    setCnhPdf(result);
  };
  const handlePickCnhPdfDrive = async () => {
    ...
    if (!result) {
      showAlert('Formato inválido', 'Somente arquivos PDF são aceitos. Selecione um PDF no Google Drive.');
      return;
    }
    setCnhPdf(result);
  };
  const handlePickCnhPdfGallery = async () => {
    const result = await pickPdfFromGallery();
    if (!result) {
      showAlert('Formato inválido', 'Somente arquivos PDF são aceitos. Selecione um PDF do dispositivo.');
      return;
    }
    setCnhPdf(result);
  };
  const handlePickSelfieCamera = async () => {
    ...
    else showAlert('Permissao necessaria', 'Permita o acesso a camera frontal para tirar a selfie.');
  };
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        >
          ...
          <View style={styles.section}>
            ...
            <TouchableOpacity style={styles.resendBtn} onPress={handleResendOTP} disabled={loading || uploading} activeOpacity={0.7}>
      ...
      >
        ...
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoText}>
            Preencha os dados, envie CNH (física ou digital) e selfie, aceite os termos e confirme seu e-mail. Em seguida será direcionado ao pagamento.
          </Text>
        </View>
        ...
        {/* Documentos CNH + Selfie — visíveis desde a primeira tela */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>
            <MaterialIcons name="verified-user" size={16} color={Colors.primary} /> Documentos para verificação
          </Text>
          <View style={styles.cnhTypeRow}>
            ...
            >
              ...
              <Text style={[styles.cnhTypeBtnText, cnhType === 'physical' && styles.cnhTypeBtnTextActive]}>CNH Física</Text>
              <Text style={[styles.cnhTypeBtnSub, cnhType === 'physical' && { color: Colors.white + 'AA' }]}>Frente + Verso (câmera)</Text>
            </TouchableOpacity>
            <TouchableOpacity
            ...
            >
              ...
              <Text style={[styles.cnhTypeBtnText, cnhType === 'digital' && styles.cnhTypeBtnTextActive]}>CNH Digital</Text>
              <Text style={[styles.cnhTypeBtnSub, cnhType === 'digital' && { color: Colors.white + 'AA' }]}>PDF (Drive/galeria)</Text>
            </TouchableOpacity>
          </View>
          {cnhType === 'physical' ? (
            <>
              <View style={docStyles.formatBadge}>
                ...
                <Text style={docStyles.formatBadgeText}>Somente câmera — tire foto da frente e do verso. Galeria não permitida.</Text>
              </View>
              <CnhPhysicalCameraBtn label="Frente da CNH *" sublabel="Foto da frente do documento" preview={cnhFront?.uri} onCamera={handlePickCnhFrontCamera} />
              <View style={{ height: Spacing.sm }} />
              <CnhPhysicalCameraBtn label="Verso da CNH *" sublabel="Foto do verso do documento" preview={cnhBack?.uri} onCamera={handlePickCnhBackCamera} />
            </>
          ) : (
            <>
              <View style={docStyles.formatBadge}>
                <MaterialIcons name="picture-as-pdf" size={14} color={Colors.error} />
                <Text style={docStyles.formatBadgeText}>Somente PDF — Drive ou galeria. Imagens não são aceitas.</Text>
              </View>
              <PdfPickerBtn
                label="Arquivo PDF da CNH Digital *"
                sublabel="Formato PDF oficial"
                preview={cnhPdf ? cnhPdf.name : null}
                onGoogleDrive={handlePickCnhPdfDrive}
                onGallery={handlePickCnhPdfGallery}
              />
            </>
          )}
          <View style={{ height: Spacing.md }} />
          <SelfiePickerBtn label="Selfie *" sublabel="Foto do rosto — câmera frontal" preview={selfie?.uri} onCamera={handlePickSelfieCamera} />
        </View>
        ...
        {/* LGPD — consentimento na primeira tela */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>
            <MaterialIcons name="security" size={16} color={Colors.primary} /> Consentimento LGPD
          </Text>
          <View style={styles.termBox}>
            <Text style={styles.termText}>
              {'"'}Autorizo o {APP_NAME} a coletar, armazenar e utilizar meus dados e documentos (CNH, selfie) para verificação de identidade e cumprimento da LGPD.{'"'}
            </Text>
          </View>
          <TouchableOpacity style={styles.checkRow} onPress={() => setLgpdAccepted(!lgpdAccepted)} activeOpacity={0.8}>
            ...
            <Text style={styles.checkLabel}>Concordo com o tratamento dos meus dados conforme descrito acima.</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.checkRow} onPress={() => setPrivacyAccepted(!privacyAccepted)} activeOpacity={0.8}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.checkLabel}>
                Li e aceito a{' '}
                <Text style={styles.linkText} onPress={() => router.push('/privacy-policy')}>Política de Privacidade</Text>
                {' '}do {APP_NAME}.
              </Text>
            </View>
          </TouchableOpacity>
