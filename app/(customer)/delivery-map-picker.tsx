...
function getAndroidGoogleMapsKey(): string {
  ...
  const fromProcess = process.env.EXPO_PUBLIC_GOOGLE_MAPS_KEY;
  ...
  return fromProcess || fromManifest || fromExtra || 'AIzaSyBc8HQ1X4uw6oGHRdzVsquazBQd2G4ofqs';
}
...
export default function DeliveryMapPickerScreen() {
  ...
  const onRegionChangeComplete = useCallback((newRegion: any) => {
    ...
    refreshPreview(latitude, longitude);
  }, [refreshPreview]);

  const onConfirm = async () => {
    setConfirmLoading(true);
    try {
      console.log('[MapPicker] Confirmando coordenadas:', coord);
      const g = await reverseGeocode(coord.latitude, coord.longitude);
      console.log('[MapPicker] Geocode obtido:', g?.formatted_address);
      ...
      console.error('[MapPicker] Erro ao confirmar:', err);
      showAlert('Endereço', 'Não foi possível confirmar este ponto. Tente novamente.');
    } finally {
    ...
    <View style={styles.root}>
      {!initDone ? (
        ...
        <View style={{ flex: 1 }}>
          ...
          />
          {/* Marcador fixo no centro da tela - Estilo iFood */}
