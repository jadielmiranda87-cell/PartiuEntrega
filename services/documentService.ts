...
/** Abre câmera frontal para selfie — sem opção de galeria */
export async function pickSelfieFromCamera(): Promise<{ uri: string; type: string } | null> {
  const granted = await requestCameraPermission();
  if (!granted) return null;
...
/** Opens Google Drive file picker. On Android, the system picker includes Drive. */
export async function pickPdfFromGoogleDrive(): Promise<{ uri: string; name: string } | null> {
  return pickPdfDocument();
}

/** Importa PDF do dispositivo (galeria/arquivos). Somente formato PDF. */
export async function pickPdfFromGallery(): Promise<{ uri: string; name: string } | null> {
  return pickPdfDocument();
}
