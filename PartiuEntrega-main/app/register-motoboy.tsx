...
import { requestAllMotoboyPermissions, MotoboyPermissionsStatus } from '@/services/permissionsService';
import { Colors, Spacing, FontSize, BorderRadius } from '@/constants/theme';
import { useSafeAreaInsets } from 'react-native-safe-area-context';

const APP_NAME = 'PartiuEntrega';

type Step = 'permissions' | 'form' | 'otp' | 'documents' | 'lgpd';
...
// ── Image Picker Button (CNH Física) ─────────────────────────────────────────
  ...
  return (
    <View style={docStyles.wrapper}>
      ...
      <TouchableOpacity style={docStyles.pdfBtn} onPress={onPdf} activeOpacity={0.8}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={docStyles.pdfBtnTitle}>Buscar na galeria (PDF)</Text>
          <Text style={docStyles.pdfBtnSub}>Selecione um PDF oficial</Text>
        </View>
...
export default function RegisterMotoboyScreen() {
  ...
  const [permissionsStatus, setPermissionsStatus] = useState<MotoboyPermissionsStatus | null>(null);
  ...
  const handleRequestPermissions = async () => {
    ...
    try {
      const status = await requestAllMotoboyPermissions();
      ...
      if (!allGranted) {
        showAlert(
          ...
          'Para continuar, conceda todas as permissões solicitadas para o app funcionar corretamente.'
        );
        return;
      }
  ...
  const handleRegister = async () => {
    ...
    if (!selfie) {
      showAlert('Selfie obrigatória', 'Tire uma selfie (câmera frontal) para validar seu cadastro.');
      return;
    }
  ...
  if (step === 'permissions') {
    ...
    if (permissionsStatus) {
      if (!permissionsStatus.camera) missing.push('Câmera');
      if (!permissionsStatus.mediaLibrary) missing.push('Arquivo de mídia');
      if (!permissionsStatus.location) missing.push('Localização (em uso)');
      if (!permissionsStatus.backgroundLocation) missing.push('Localização em segundo plano');
      if (!permissionsStatus.notifications) missing.push('Notificações');
    }
        ...
        >
          ...
          <View style={styles.infoCard}>
            ...
            <Text style={styles.infoText}>
              Antes do cadastro, precisamos das permissões para garantir segurança, enviar documentos e calcular rotas.
            </Text>
          </View>

          <View style={styles.section}>
            <Text style={styles.sectionTitle}>O que vamos solicitar</Text>
            <PermissionRow icon="camera-alt" title="Câmera" desc="Para capturar fotos da CNH e selfie." />
            <PermissionRow icon="photo-library" title="Arquivo de mídia" desc="Para selecionar fotos (CNH física)." />
            <PermissionRow icon="location-on" title="Localização (em uso)" desc="Para calcular rotas de entrega e encontrar motoboys próximos." />
            <PermissionRow icon="location-on" title="Localização em segundo plano" desc="Para acompanhar entregas mesmo com o app minimizado." />
            <PermissionRow icon="notifications" title="Notificações" desc="Para alertar sobre novos pedidos e mudanças de status." />
          </View>
          ...
          {permissionsStatus ? (
            <View style={styles.section}>
              ...
              {missing.length === 0 ? (
                <Text style={{ color: Colors.success, fontWeight: '700' }}>Todas as permissões foram concedidas.</Text>
              ) : (
                <Text style={{ color: Colors.error }}>Faltando: {missing.join(', ')}.</Text>
              )}
            </View>
          ) : null}

          <TouchableOpacity
            style={[styles.btn, requestingPermissions && styles.btnDisabled]}
          ...
          >
            {requestingPermissions ? (
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                ...
                <Text style={styles.btnText}>Permitir e continuar</Text>
              </View>
            )}
          </TouchableOpacity>
        </ScrollView>
    ...
    return (
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        >
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Permissões verificadas</Text>
            <Text style={styles.infoText}>
              As permissões foram solicitadas antes do início do cadastro.
            </Text>
          </View>
        ...
        >
          ...
          {cnhType === 'physical' ? (
            <View style={styles.section}>
              ...
              <ImagePickerBtn
                label="Frente da CNH *"
                sublabel="Foto clara da frente do documento fisico (somente câmera)"
              ...
              <ImagePickerBtn
                label="Verso da CNH *"
                sublabel="Foto clara do verso do documento fisico (somente câmera)"
                preview={cnhBack?.uri}
          ...
          <View style={styles.section}>
            ...
            <SelfiePickerBtn
              label="Selfie *"
              sublabel="Foto do rosto — somente camera frontal, sem galeria"
              preview={selfie?.uri}
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      >
        ...
        {/* Selfie (validação extra) */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>
            <MaterialIcons name="face" size={16} color={Colors.primary} /> Selfie
          </Text>
          <SelfiePickerBtn
            label="Selfie para validar *"
            sublabel="Somente câmera frontal — sem galeria"
            preview={selfie?.uri}
            onCamera={handlePickSelfieCamera}
          />
        </View>
