...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { address, place_id } = await req.json();
    ...
    if (place_id) {
      ...
      return new Response(JSON.stringify({ error: 'address or place_id required' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    console.log(`Geocode "${address || place_id}" → ${result.geometry.location.lat},${result.geometry.location.lng}`);
