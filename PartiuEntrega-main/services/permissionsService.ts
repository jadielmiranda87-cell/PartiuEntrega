 ...
 * Usada para manter o rastreio/atualizações do motoboy mesmo com o app minimizado.
 */
export async function requestBackgroundLocationPermission(): Promise<PermissionResult> {
  ...
  if (!existing.canAskAgain) {
    return {
      granted: false,
      reason:
        'Permissão de localização em segundo plano negada permanentemente. Acesse Configurações > PartiuEntrega > Localização e habilite "Sempre".',
    };
  }

  // Para background dar certo, a permissão de uso em primeiro plano deve estar ativa.
  const foreground = await Location.getForegroundPermissionsAsync();
  if (foreground.status !== 'granted') {
    return {
      granted: false,
      reason:
        'Ative a localização "Ao usar o app" antes de habilitar a localização em segundo plano.',
    };
  }
  ...
  return {
    granted: false,
    reason:
      'Localização em segundo plano necessária para manter entregas em andamento e atualizações em tempo real.',
  };
}
...
export interface MotoboyPermissionsStatus {
  ...
  location: boolean; // Localização (em uso / foreground)
  backgroundLocation: boolean; // Localização em segundo plano
...
export async function requestAllMotoboyPermissions(): Promise<MotoboyPermissionsStatus> {
  // Solicita permissões que não dependem umas das outras em paralelo.
  const [camera, media, notifications] = await Promise.allSettled([
    requestCameraPermission(),
    requestMediaLibraryPermission(),
    requestNotificationPermission(),
  ]);

  // Localização: solicita foreground antes do background.
  const location = await requestLocationPermission();
  const backgroundLocation = await requestBackgroundLocationPermission();

  return {
    camera: camera.status === 'fulfilled' && camera.value.granted,
    mediaLibrary: media.status === 'fulfilled' && media.value.granted,
    location: location.granted,
    backgroundLocation: backgroundLocation.granted,
    notifications: notifications.status === 'fulfilled' && notifications.value.granted,
  };
}
